...
| `run_lda_kl_jsonl_check.py` + `config_lda_kl_jsonl_check.yaml` | **GPU:** tiny `concurrency_speed_tests` run (3 prompts, c=1) with `collect_kl` + `save_generations`; verifies `mean_kl_divergence` / `max_kl_divergence` in `generations.jsonl`. Run: `python test/basic_func/run_lda_kl_jsonl_check.py` from repo root. |
